...
# Equation (19) is expressed in Lagrangian coordinates; here, we rewrite it in Eulerian coordinates.
# We take the physically useful solution ``c_p^-`` and rewrite it
# to avoid cancellation for small ``k`` or large ``\lambda``.
